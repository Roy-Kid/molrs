    ...
    for (row_i, line) in lines.iter().enumerate() {
        ...
        for (buf_idx, (_, ty)) in cols.iter().enumerate() {
            ...
            match (&mut buffers[buf_idx], ty) {
                ...
                (ColBuf::I(v), PropType::I) => v.push(tok.parse::<i64>().map_err(|_| {
                    format!("line {} col {}: invalid int '{}", row_i, buf_idx, tok)
                })?),
                (ColBuf::R(v), PropType::R) => v.push(tok.parse::<f32>().map_err(|_| {
    ...
    for ((name, ty), buf) in cols.into_iter().zip(buffers.into_iter()) {
        match (ty, buf) {
            ...
            (PropType::R, ColBuf::R(v)) => {
                // Store as f32
                let arr: ArrayD<f32> = Array1::from_vec(v).into_dyn();
                block.insert(name, arr).map_err(|e| e.to_string())?;
            }
...
";
    ...
    fn test_xyz_frame_writer() {
        ...
        let h = ndarray::array![[10.0_f32, 0.0, 0.0], [0.0, 10.0, 0.0], [0.0, 0.0, 10.0],];
        let origin = ndarray::array![0.0_f32, 0.0, 0.0];
    ...
    fn test_write_read_xyz_roundtrip() {
        ...
        let x = Array1::from_vec(vec![0.0_f32, 1.0, -1.0])
            .into_shape_with_order(IxDyn(&[n]))
            .unwrap()
            .into_dyn();
        let y = Array1::from_vec(vec![0.5_f32, -0.5, 2.0])
            .into_shape_with_order(IxDyn(&[n]))
            .unwrap()
            .into_dyn();
        let z = Array1::from_vec(vec![1.5_f32, 2.5, -2.5])
            .into_shape_with_order(IxDyn(&[n]))
            .unwrap()
            .into_dyn();
        ...
        for i in 0..n {
            assert!((x_rt[i] - [0.0_f32, 1.0, -1.0][i]).abs() < 1e-3);
            assert!((y_rt[i] - [0.5_f32, -0.5, 2.0][i]).abs() < 1e-3);
            assert!((z_rt[i] - [1.5_f32, 2.5, -2.5][i]).abs() < 1e-3);
        }
    }
}
    ...
    for priority_k in &priority_keys {
            ...
            && let Some(col) = atoms.get(priority_k)
        {
            ...
            let type_char = match col.dtype() {
                crate::core::block::DType::F32 | crate::core::block::DType::F64 => "R",
                crate::core::block::DType::I64
                | crate::core::block::DType::U32
                | crate::core::block::DType::U8 => "I",
                crate::core::block::DType::Bool => "L",
                crate::core::block::DType::String => "S",
            };
