...
//! let x = atoms.get_f32("x").expect("x coordinates");
//!
//! // Write to new file
...
fn parse_atoms<R: BufRead>(
    ...
    while atoms.len() < num_atoms {
        ...
        let atom = if tokens.len() == 5 {
            // atomic style
            AtomData {
                id: tokens[0].parse().map_err(err_mapper)?,
                molecule_id: None,
                atom_type: tokens[1].to_string(),
                charge: None,
                x: tokens[2].parse().map_err(err_mapper)?,
                y: tokens[3].parse().map_err(err_mapper)?,
                z: tokens[4].parse().map_err(err_mapper)?,
            }
        } else if tokens.len() == 6 {
            // Could be charge or molecular - try to detect
            // If token[2] parses as float, it's charge style
            if tokens[2].parse::<f32>().is_ok() {
                // charge style
                AtomData {
                    id: tokens[0].parse().map_err(err_mapper)?,
                    molecule_id: None,
                    atom_type: tokens[1].to_string(),
                    charge: Some(tokens[2].parse().map_err(err_mapper)?),
                    x: tokens[3].parse().map_err(err_mapper)?,
                    y: tokens[4].parse().map_err(err_mapper)?,
                    z: tokens[5].parse().map_err(err_mapper)?,
                }
            } else {
                // molecular style
                AtomData {
                    id: tokens[0].parse().map_err(err_mapper)?,
                    molecule_id: Some(tokens[1].parse().map_err(err_mapper)?),
            ...
            AtomData {
                ...
                molecule_id: Some(tokens[1].parse().map_err(err_mapper)?),
                atom_type: tokens[2].to_string(),
                charge: Some(tokens[3].parse().map_err(err_mapper)?),
                x: tokens[4].parse().map_err(err_mapper)?,
                y: tokens[5].parse().map_err(err_mapper)?,
                z: tokens[6].parse().map_err(err_mapper)?,
            }
        } else {
...
fn parse_bonds<R: BufRead>(
    ...
    while bonds.len() < num_bonds {
        ...
        let bond = BondData {
            _id: tokens[0].parse().map_err(err_mapper)?,
            bond_type: tokens[1].to_string(),
            atom_i: tokens[2].parse().map_err(err_mapper)?,
            atom_j: tokens[3].parse().map_err(err_mapper)?,
        };
...
fn parse_angles<R: BufRead>(
    ...
    while angles.len() < num_angles {
        ...
        let angle = AngleData {
            ...
            atom_i: tokens[2].parse().map_err(err_mapper)?,
            atom_j: tokens[3].parse().map_err(err_mapper)?,
            atom_k: tokens[4].parse().map_err(err_mapper)?,
        };
...
fn parse_dihedrals<R: BufRead>(
    ...
    while dihedrals.len() < num_dihedrals {
        ...
        let dihedral = DihedralData {
            ...
            atom_j: tokens[3].parse().map_err(err_mapper)?,
            atom_k: tokens[4].parse().map_err(err_mapper)?,
            atom_l: tokens[5].parse().map_err(err_mapper)?,
        };
...
fn build_frame(
    ...
    if !atoms.is_empty() {
        ...
        for (idx, atom) in atoms.iter().enumerate() {
            ...
            let type_num = if let Ok(num) = atom.atom_type.parse::<i64>() {
                num
            } else {
            ...
            atom_id_map.insert(atom.id, idx as i64);
        }

        // Insert coordinate arrays
        let to_arr_f32 = |v: Vec<f32>| -> std::io::Result<ndarray::ArrayD<f32>> {
            Ok(Array1::from_vec(v)
                .into_shape_with_order(IxDyn(&[n]))
                .map_err(err_mapper)?
                .into_dyn())
        };
        let to_arr_i64 = |v: Vec<i64>| -> std::io::Result<ndarray::ArrayD<i64>> {
            Ok(Array1::from_vec(v)
                .into_shape_with_order(IxDyn(&[n]))
                .map_err(err_mapper)?
                .into_dyn())
        ...
        atom_block
            .insert("x", to_arr_f32(x_vec)?)
            .map_err(err_mapper)?;
        atom_block
            .insert("y", to_arr_f32(y_vec)?)
            .map_err(err_mapper)?;
        atom_block
            .insert("z", to_arr_f32(z_vec)?)
            .map_err(err_mapper)?;
        atom_block
            .insert("id", to_arr_i64(id_vec)?)
            .map_err(err_mapper)?;
        atom_block
            .insert("type", to_arr_i64(type_vec)?)
            .map_err(err_mapper)?;

        if has_charge {
            atom_block
                .insert("charge", to_arr_f32(charge_vec)?)
                .map_err(err_mapper)?;
        }
        if has_mol_id {
            atom_block
                .insert("molecule_id", to_arr_i64(mol_id_vec)?)
                .map_err(err_mapper)?;
        }
        ...
        if !bonds.is_empty() {
            ...
            for bond in &bonds {
                ...
                let type_num = if let Ok(num) = bond.bond_type.parse::<i64>() {
                    num
                } else {
                    bond_type_labels
                        .iter()
            ...
            bond_block
                .insert("atom_i", mk_barr(atom_i_vec)?)
                .map_err(err_mapper)?;
            bond_block
                .insert("atom_j", mk_barr(atom_j_vec)?)
                .map_err(err_mapper)?;
            bond_block
                .insert("type", mk_barr(bond_type_vec)?)
                .map_err(err_mapper)?;
        ...
        if !angles.is_empty() {
            ...
            for angle in &angles {
                ...
                let type_num = if let Ok(num) = angle.angle_type.parse::<i64>() {
                    num
                } else {
                    angle_type_labels
                        .iter()
            ...
            angle_block
                .insert("atom_i", mk_aarr(atom_i_vec)?)
                .map_err(err_mapper)?;
            angle_block
                .insert("atom_j", mk_aarr(atom_j_vec)?)
                .map_err(err_mapper)?;
            angle_block
                .insert("atom_k", mk_aarr(atom_k_vec)?)
                .map_err(err_mapper)?;
            angle_block
                .insert("type", mk_aarr(angle_type_vec)?)
                .map_err(err_mapper)?;
        ...
        if !dihedrals.is_empty() {
            ...
            for dihedral in &dihedrals {
                ...
                let type_num = if let Ok(num) = dihedral.dihedral_type.parse::<i64>() {
                    num
                } else {
            ...
            dihedral_block
                .insert("atom_i", mk_darr(atom_i_vec)?)
                .map_err(err_mapper)?;
            dihedral_block
                .insert("atom_j", mk_darr(atom_j_vec)?)
                .map_err(err_mapper)?;
            dihedral_block
                .insert("atom_k", mk_darr(atom_k_vec)?)
                .map_err(err_mapper)?;
            dihedral_block
                .insert("atom_l", mk_darr(atom_l_vec)?)
                .map_err(err_mapper)?;
            dihedral_block
                .insert("type", mk_darr(dihedral_type_vec)?)
                .map_err(err_mapper)?;
...
impl<W: Write> FrameWriter for LAMMPSDataWriter<W> {
    fn write_frame(&mut self, frame: &Frame) -> std::io::Result<()> {
        ...
        let num_bond_types = if let Some(bonds) = bonds {
            if let Some(bond_types) = bonds.get_i64("type") {
        ...
        let has_charge = atoms.get_f32("charge").is_some();
        let has_mol_id = atoms.get_i64("molecule_id").is_some();
        ...
        for i in 0..num_atoms {
            ...
            if has_mol_id {
                let mol_ids = atoms.get_i64("molecule_id").unwrap();
            ...
            if has_charge {
                let charges = atoms.get_f32("charge").unwrap();
