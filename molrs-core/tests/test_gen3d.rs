//! Integration tests for Gen3D generation.

use molrs::{
    Atom, AtomId, EmbedAlgorithm, ForceFieldKind, Gen3DOptions, MolGraph, PropValue, StageKind,
    generate_3d,
};
...
fn bond(g: &mut MolGraph, a: AtomId, b: AtomId, order: f64) {
...
fn test_generate_3d_ethanol_assigns_coordinates() {
    ...
    let o = g.add_atom(atom("O"));
    bond(&mut g, c1, c2, 1.0);
    bond(&mut g, c2, o, 1.0);

    let opts = Gen3DOptions {
        rng_seed: Some(42),
        ..Default::default()
    };
    let (out, report) = generate_3d(&g, &opts).expect("gen3d");
...
fn test_generate_3d_seed_reproducible() {
    ...
    let mut g = MolGraph::new();
    let a = g.add_atom(atom("C"));
    let b = g.add_atom(atom("C"));
    let c = g.add_atom(atom("C"));
    let d = g.add_atom(atom("C"));
    bond(&mut g, a, b, 1.0);
    bond(&mut g, b, c, 1.0);
    bond(&mut g, c, d, 1.0);
    ...
    let (g1, _) = generate_3d(&g, &opts).expect("first gen3d");
    let (g2, _) = generate_3d(&g, &opts).expect("second gen3d");
...
fn test_generate_3d_auto_forcefield_reports_fallback_warning() {
    ...
    let (_out, report) = generate_3d(&g, &opts).expect("gen3d");
...
fn test_generate_3d_empty_molecule_returns_error() {
    let g = MolGraph::new();
    let err = generate_3d(&g, &Gen3DOptions::default()).expect_err("empty must error");
    assert!(
...
fn test_generate_3d_distance_geometry_assigns_coordinates() {
    ...
    let (out, report) = generate_3d(&g, &opts).expect("distance geometry gen3d");
    assert_eq!(
...
fn test_fragment_rules_uses_ring_template_on_benzene() {
    let mut g = MolGraph::new();
    let ids: Vec<_> = (0..6).map(|_| g.add_atom(atom("C"))).collect();
    for i in 0..6 {
    ...
    let (out, report) = generate_3d(&g, &opts).expect("benzene gen3d");
