...
fn test_read_labelmap() {
    ...
    let _y = atoms.get_f32("y").expect("y coordinates");
    let _z = atoms.get_f32("z").expect("z coordinates");
    let types = atoms.get_i64("type").expect("atom types");
    let _charges = atoms.get_f32("charge").expect("charges");
    let mol_ids = atoms.get_i64("molecule_id").expect("molecule IDs");
    ...
    let atom_i = bonds.get_i64("atom_i").expect("atom_i");
    let _atom_j = bonds.get_i64("atom_j").expect("atom_j");
    let _bond_types = bonds.get_i64("type").expect("bond types");
...
fn test_write_and_read_roundtrip() {
    ...
    atoms
        ...
        .unwrap();
    atoms
        .insert("z", Array1::from_vec(vec![0.0_f32, 0.0, 0.0]).into_dyn())
        .unwrap();
    atoms
        .insert("id", Array1::from_vec(vec![1_i64, 2, 3]).into_dyn())
        .unwrap();
    atoms
        .insert("type", Array1::from_vec(vec![1_i64, 1, 2]).into_dyn())
        .unwrap();
    atoms
        .insert(
            "charge",
            Array1::from_vec(vec![0.0_f32, -1.0, 1.0]).into_dyn(),
        )
        .unwrap();
    atoms
        .insert(
            "molecule_id",
            Array1::from_vec(vec![1_i64, 1, 1]).into_dyn(),
        )
        .unwrap();
    ...
    bonds
        .insert("atom_i", Array1::from_vec(vec![0_i64, 1]).into_dyn())
        .unwrap();
    bonds
        .insert("atom_j", Array1::from_vec(vec![1_i64, 2]).into_dyn())
        .unwrap();
    bonds
        .insert("type", Array1::from_vec(vec![1_i64, 1]).into_dyn())
        .unwrap();
    ...
    let x2 = atoms2.get_f32("x").expect("x coordinates");
    assert_eq!(x2[0], 1.0);
    assert_eq!(x2[1], 2.0);
    assert_eq!(x2[2], 3.0);
...
fn test_write_with_type_labels() {
    ...
    atoms
        ...
        .unwrap();
    atoms
        .insert("y", Array1::from_vec(vec![0.0_f32, 1.0]).into_dyn())
        .unwrap();
    atoms
        .insert("z", Array1::from_vec(vec![0.0_f32, 0.0]).into_dyn())
        .unwrap();
    atoms
        .insert("id", Array1::from_vec(vec![1_i64, 2]).into_dyn())
        .unwrap();
    atoms
        .insert("type", Array1::from_vec(vec![1_i64, 2]).into_dyn())
        .unwrap();
...
fn test_read_angles_and_dihedrals() {
    ...
    let atom_i = angles.get_i64("atom_i").expect("atom_i");
    let atom_j = angles.get_i64("atom_j").expect("atom_j");
    let atom_k = angles.get_i64("atom_k").expect("atom_k");
    let angle_types = angles.get_i64("type").expect("angle types");
    ...
    let atom_i = dihedrals.get_i64("atom_i").expect("atom_i");
    let atom_j = dihedrals.get_i64("atom_j").expect("atom_j");
    let atom_k = dihedrals.get_i64("atom_k").expect("atom_k");
    let atom_l = dihedrals.get_i64("atom_l").expect("atom_l");
    let dihedral_types = dihedrals.get_i64("type").expect("dihedral types");
...
fn test_read_solvated_system() {
    ...
    let x = atoms.get_f32("x").expect("x coordinates");
    let y = atoms.get_f32("y").expect("y coordinates");
    let z = atoms.get_f32("z").expect("z coordinates");
