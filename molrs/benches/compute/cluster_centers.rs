//! `ClusterCenters::compute` — MIC-aware geometric cluster centers
//! across size + frame axes.

use criterion::{BenchmarkId, Criterion, Throughput, criterion_group};
use molrs::compute::cluster_centers::ClusterCenters;
use molrs::compute::traits::Compute;

use crate::helpers;

fn size_sweep(c: &mut Criterion) {
    let mut group = c.benchmark_group("cluster_centers/size_sweep");
...
fn frame_sweep(c: &mut Criterion) {
    let mut group = c.benchmark_group("cluster_centers/frame_sweep");
    helpers::configure(&mut group);
    let centers = ClusterCenters::new();
