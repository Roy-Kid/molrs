//! `Cluster::compute` — distance-based BFS clustering across size + frame axes.
...
fn size_sweep(c: &mut Criterion) {
    let mut group = c.benchmark_group("cluster/size_sweep");
...
fn frame_sweep(c: &mut Criterion) {
    let mut group = c.benchmark_group("cluster/frame_sweep");
    helpers::configure(&mut group);
    let cluster = Cluster::new(2);
