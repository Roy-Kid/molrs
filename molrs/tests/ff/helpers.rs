...
/// relative to this crate (the workspace root). Mirrors the io test target's
/// `common::tests_data_dir`.
...
/// Path to a file inside `tests-data/`, e.g. `data_path("xml/oplsaa.xml")`.
pub fn data_path(relative: &str) -> PathBuf {
    tests_data_dir().join(relative)
}

/// Every file directly inside `tests-data/<format>/`, sorted.
