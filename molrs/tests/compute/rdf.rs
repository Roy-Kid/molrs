...
/// adjacent bin).
fn peak_near(result: &molrs::compute::rdf::RDFResult, r: F, tol: F) -> F {
    result
        .bin_centers
        .iter()
        .zip(result.rdf.iter())
...
fn counts_near(result: &molrs::compute::rdf::RDFResult, r: F, tol: F) -> F {
    result
        .bin_centers
        .iter()
        .zip(result.n_r.iter())
...
fn empty_frames_is_error() {
    ...
    assert!(matches!(
        err,
        molrs::compute::error::ComputeError::EmptyInput
    ));
}
