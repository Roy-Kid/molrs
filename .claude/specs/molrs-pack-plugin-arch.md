...
**Phase B — 公开新 trait，并行新旧 API**（v2-r2 修订：Constraint 与 Restraint 合一；B.4 取消）
- [ ] **B.1** 在 `molrs-pack` 公开 `Constraint` / `Region` / `Selector` trait；`pub type Restraint = Constraint;` 作兼容名
- [ ] **B.2** opaque builder：`InsideBox::new(...)` 返回不暴露 enum 的构造器，内部填 `BuiltinConstraint`
- [ ] **B.3** `Molpack::add_constraint(impl Into<ConstraintInput>)` — 接受 builder / `Box<dyn Constraint>` / Python；**实现 = 遍历 targets 各调一次 `with_constraint`**（global ≡ per-target × N）
...
- [ ] **B.6** 验收：用户能用纯 Rust 写自定义 Constraint（global 或 per-target 两种挂法都跑通）；基线无退化

**Phase C — Python 桥**
- [ ] **C.1** `molrs-python` 新建 `pack` 子模块
- [ ] **C.2** 暴露 `Constraint` / `Handler` / `Relaxer` Python ABC（`Restraint` 作为 `Constraint` 的别名同时 `pub use` 导出）
- [ ] **C.3** PyO3 桥（`PyConstraintImpl: Constraint`），`is_parallel_safe = false`
- [ ] **C.4** 编译时分区（`ConstraintSet` 三段 CSR）+ 单 GIL 批量调用
- [ ] **C.5** Python e2e：自定义 Python Constraint（global + per-target 两种 scope 各跑一次）跑通完整 packing
...
8. **`Constraint` 与 `Restraint` 本质无区别**（v2 第二轮修订，A.7 后定案）：两者都是 "penalty function + 梯度" 的 trait，数学上同型。它们唯一的差别是 **作用域 (scope)**，不是类型：
   - **Global scope**（`Molpack::add_constraint(c)`）≡ 语义等价于对每个 `Target` 各调一次 `target.with_constraint(c.clone())`
   - **Per-target scope**（`Target::with_constraint(c)` / `Target::with_constraint_for_atoms(...)`）仅挂到该 target 的 atoms 上
   - 区分 "硬约束必须到 0" vs "软偏置可以有残差" 在 v1/v2 spec 原计划拆成两个 trait，实际上这是**实例配置级别的差别**（是否进 `frest` 精度判据 / 用 `scale` 还是 `scale2` / weight 值），不是类型级别的差别。**只保留一个 trait**（命名延续 `Constraint`；`Restraint` 作为 type alias 保留为兼容名）。
...
2. `Constraint`（v1 曾拆出的 `Restraint` 与之同型，只保留一个 trait）既可 **global**（`Molpack::add_constraint`，语义 = 给每个 target 都挂一份）也可 **per-target**（`Target::with_constraint`）。
3. 支持 **Python 注入** Constraint / Handler / Relaxer，无需重新编译 Rust。
...
## 4. 术语：三类可扩展点（v2 第二轮修订）

| 维度 | **Constraint**（= 旧 Restraint；同型） | **Relaxer** | **Handler** |
|---|---|---|---|
...
| 对应 Packmol 概念 | `restraint kind 1..15` + `tolerance`；硬约束总是进 `frest` | （无原生） | 打印/进度 |
| 是否允许 Python 实现 | 允许但慢，文档告警（`is_parallel_safe=false` 强制单线程） | 允许 | 推荐 |
| 现有代码中的对应 | `BuiltinConstraint{kind,params[9]}`（A.2 rename，原名 `Restraint`）+ `iratom_*` CSR | `Relaxer` / `RelaxerRunner`（A.3 rename，原名 `Hook`） | `handler.rs` 的 `Handler` |
| v2 名字 | `Constraint`（`Restraint` 保留为 type alias） | `Relaxer`（支持 MC/MD/梯度下降等多策略） | `Handler`（扩展） |

**Constraint 与 Restraint 统一说明**：数学上都是 "penalty function + 梯度"，**类型上无区别**。v1 spec 曾按 "硬 vs 软" 拆两 trait，v2 修订后合并：

- 旧 `struct Restraint{kind: u8}` → 内部重命名 `BuiltinConstraint`（A.2 已落地）
- 唯一 `trait Constraint` 统一覆盖内置 + 用户扩展（Rust）+ Python；`type Restraint = Constraint;` 作兼容名
- "硬 vs 软" 的区分下放到**实例配置**：是否进 `frest` 精度判据 / 用 `scale` 还是 `scale2` / `weight` 值，由具体实现决定，不由类型决定

**Scope 等价律（重要）**：

```rust
// 这两者语义完全等价
molpack.add_constraint(c);
for t in &mut all_targets { t.with_constraint(c.clone()); }
```

`Molpack::add_constraint(c)` 内部就是遍历 `targets` 并 `target.with_constraint(c.clone())`；没有单独的 "global constraint" 路径，也不会在 `PackContext` 里另开一个全局 slot。Atom 级同理：`Target::with_constraint_for_atoms(idx, c)` 是已有能力，scope 最细到 atom 子集。
...
| 2 | `Constraint` (`= Restraint`) | trait | 唯一的 penalty+gradient 抽象；global 或 per-target 均可挂；内置 + 用户 Rust + Python | `molrs-pack/src/constraint/` |
...
| 6 | `Selector` | trait | 选原子子集（编译时 resolve 成 `Vec<u32>`） | `molrs-pack/src/constraint/selector.rs` |
...
### 6.4 Restraint (= Constraint；无独立 trait)

v2 第二轮修订：**不引入独立的 `Restraint` trait**。v1/v2-r1 打算拆出 `Restraint`（软偏置，多原子 selector）是被"软 vs 硬"这个实例配置维度错误地抬升到了类型维度的产物。

要点：

- 用户视角有 `type Restraint = Constraint;` 作兼容名（熟悉 Packmol 术语的人仍可写 `Restraint`），但两者是同一个 trait。
- §6.2 的 `Constraint` trait 已经覆盖 per-atom penalty + 梯度。需要多原子联合 penalty（例如 `DistanceRestraint(a, b, target, k)`）时，实现方把 selector 放到自己结构体内部（`self.atoms: Vec<u32>`），在 `f` / `fg` 里自己遍历；**不**把 `selector()` 提到 trait。
- `weight` 同理，作为实例字段（`k` 系数）由实现方自持，**不**作为 trait 方法。
- 多原子约束的梯度累加契约：`fg` 签名保持单原子 `g: &mut [F;3]`——多原子累加由外层 `ConstraintSet` 调度器调 `fg` 多次完成，避免 trait 签名特化到多原子形态。

**Built-in 示例**（全部 `impl Constraint`）：

```rust
pub struct DistanceConstraint    { i: u32, j: u32, target: F, k: F }   // 挂到 atom i 时用 j 的位置
pub struct PositionConstraint    { i: u32, target: [F; 3], k: F }
pub struct OrientationConstraint { mol_id: u32, axis: [F;3], target: [F;3], k: F }
```

（命名仍可叫 `DistanceRestraint` 等，因为 `Restraint = Constraint` 是别名——挑"Restraint 字面更符合 Packmol 用户习惯"时用它，挑"Constraint 更通用"时用 Constraint，两个名都解析到同一个 trait。）
...
Constraint 有两条挂点，语义等价：

- **Global**: `Molpack::add_constraint(c)` — 内部等同于对每个 `Target` 各调一次 `with_constraint(c.clone())`
- **Per-target**: `Target::with_constraint(c)` / `Target::with_constraint_for_atoms(idx, c)` — 仅挂到该 target（可进一步限到 atom 子集）

没有"软 restraint"vs"硬 constraint"两条并列 API——都是 `add_constraint`。
...
let result = Molpack::new()
    ...
    // "软偏置"仍然走同一个 add_constraint；实例字段（k / weight / 是否进 frest）自决定
    .add_constraint(DistanceConstraint::new(atom_a, atom_b, target=1.5, k=10.0))
    .add_constraint(py_orientation_bias)
    ...
    .target(mol_b).with_constraint_for_atoms(&[0, 1, 2], LocalBiasConstraint::new(...))
...
1. 在 `molrs-pack` 公开 `Constraint` / `Region` / `Selector` trait（`type Restraint = Constraint;` 作为兼容名同时 `pub use`）
2. 内置约束保持 `BuiltinConstraint` enum；向用户暴露 `InsideBox::new(...)` 等构造器返回 **opaque builder** 类型，内部填 `BuiltinConstraint`（不暴露 enum 本身）
3. `Molpack::add_constraint(impl Into<ConstraintInput>)` — 接受三路（builder / `Box<dyn Constraint>` / Python）；**内部实现 = 遍历 targets 各调一次 `with_constraint`**（不开新存储路径）
4. ~~老的 `add_restraint(Restraint)` 标 `#[deprecated]`~~ — **不做**：`Molpack` 上从未有过 `add_restraint` 公开方法（A.2 之后 `Restraint` 也只在内部；重命名后的 `BuiltinConstraint` 只在 `Target::with_constraint` 路径使用）。没有要 deprecate 的老 API
5. `Handler` trait 加入 §6.6 的新默认方法
6. **验收**：用户能用纯 Rust 写自定义 Constraint（既可 global 挂也可 per-target 挂）并跑通；基线无退化

### Phase C：Python 桥（1-2 周）

1. 在 `molrs-python` 新建 `pack` 子模块
2. 暴露 `Constraint` / `Handler` / `Relaxer` Python 抽象基类（`Restraint` 作为 `Constraint` 的 Python 侧 alias 同时导出）
3. 实现 PyO3 桥（`PyConstraintImpl: Constraint`），`is_parallel_safe = false`
4. 实现编译时分区（`ConstraintSet` 的三段 CSR）与单 GIL 批量调用
5. 写 Python 端 e2e 测试：自定义 Python Constraint（global 或 per-target 挂）跑通完整 packing
...
**结束**。本 spec 完成后，molrs-pack 从"Packmol 的 Rust port"升级为"通用分子堆积框架"，且**性能不退化**（§10 量化保证），Python 可扩展三个弱耦合扩展点（Constraint / Handler / Relaxer）；`Constraint` 同时支持 global (`Molpack::add_constraint`) 与 per-target (`Target::with_constraint`) 两种 scope，两者语义等价。
