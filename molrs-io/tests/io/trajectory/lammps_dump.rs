...
fn all_lammps_bad_files() -> Vec<std::path::PathBuf> {
    let dir = crate::common::data_path("lammps/bad");
    let mut paths: Vec<_> = std::fs::read_dir(&dir)
...
fn test_lenient_atom_duplicated_id() {
    let path = crate::common::data_path("lammps/bad/atom-duplicated-id.lammpstrj");
...
fn test_nacl_scaled_coords() {
    let path = crate::common::data_path("lammps/nacl.lammpstrj");
...
fn test_properties_dump_has_custom_columns() {
    let path = crate::common::data_path("lammps/properties.lammpstrj");
