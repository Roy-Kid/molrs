...
use molrs::types::F;
use molrs_io::data::xyz::{read_xyz_frame, write_xyz_frame};
...
fn all_xyz_good_files() -> Vec<std::path::PathBuf> {
    let dir = crate::common::data_path("xyz");
...
fn all_xyz_bad_files() -> Vec<std::path::PathBuf> {
    let dir = crate::common::data_path("xyz/bad");
    let mut paths: Vec<_> = std::fs::read_dir(&dir)
...
fn test_extended_xyz_has_simbox() {
    let path = crate::common::data_path("xyz/extended.xyz");
...
fn test_methane_atom_count() {
    let path = crate::common::data_path("xyz/methane.xyz");
...
fn test_velocities_xyz_has_velocity_columns() {
    let path = crate::common::data_path("xyz/velocities.xyz");
